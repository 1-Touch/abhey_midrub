...
class Adminarea extends MY_Controller {
    ...
    public function facebook_user($username) 
    {
        
        $this->check_session($this->user_role, 1);
        $data['user_detail'] = $this->facebook->get_facebook_user_detail($username);


        $this->body = 'admin/facebook';
        
        $this->content = [
            'user' => $data['user_detail']['detail'],
            'user_logs' => $data['user_detail']['logs'],
    ...
    public function facebook_user_ajax() 
    {
        // Check if the session exists and if the login user is admin
        $this->check_session($this->user_role, 1);
