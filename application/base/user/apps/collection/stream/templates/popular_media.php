 ...
 * Popular Media
 *
 * This file contains the Stream's Popular Media template 
 *
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.6
...
/*
 * Class Popular_media contains the Stream's Popular Media template 
 * 
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.6
 */
class Popular_media implements MidrubBaseUserAppsCollectionStreamInterfaces\Stream {
    ...
    public function template_content( $stream, $return = FALSE ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream->stream_id, 'popular_media');
        ...
        $response = $this->fb->get(
            '/' . $default . '/top_media?fields=caption,media_url,media_type,comments_count,like_count,permalink&limit=10&user_id=' . $network_details[0]->net_id,
        ...
        if ($this->CI->input->post()) {
            ...
            if ( $this->CI->form_validation->run() !== false ) {
                ...
                if ( $this->CI->stream_tabs_model->verify_stream_owner( $this->CI->user_id, $stream_id ) ) {
                    ...
                    $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream_id, 'popular_media');
    ...
    public function stream_get_setup( $stream ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream[0]->stream_id, 'popular_media');
        ...
        $setup_data .= '<input type="hidden" name="stream-template" value="popular_media">'
    ...
    public function template_info() {
        ...
        return array (
            'displayed_name' => $this->CI->lang->line('popular_media'),
            'icon' => '<i class="icon-social-instagram"></i>',
            'description' => $this->CI->lang->line('display_most_popular_media'),
