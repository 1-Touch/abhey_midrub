 ...
 * New Tweets Template
 *
 * This file contains the Stream's New Tweets template 
 *
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.5
...
/*
 * New_tweets contains the Stream's New_tweets template 
 * 
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.5
 */
class New_tweets implements MidrubBaseUserAppsCollectionStreamInterfaces\Stream {
    ...
    public function template_content( $stream, $return = FALSE ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream->stream_id, 'new_tweets');
            ...
            foreach ( $tweets->statuses as $status ) {
                ...
                if ( @$status->entities->hashtags[0] ) {
                    ...
                    foreach ( $status->entities->hashtags as $hashtag ) {
                        ...
                        $hashtags .= '<a href="https://twitter.com/hashtag/' . $hashtag->text . '" target="_blank"' . $links_color . '>#' . $hashtag->text . '</a> ';
        ...
        if ($this->CI->input->post()) {
            ...
            if ( $this->CI->form_validation->run() !== false ) {
                ...
                if ( $this->CI->stream_tabs_model->verify_stream_owner( $this->CI->user_id, $stream_id ) ) {
                    ...
                    $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream_id, 'new_tweets');
    ...
    public function stream_get_setup( $stream ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream[0]->stream_id, 'new_tweets');
        ...
        if ( $get_setup ) {
            ...
            foreach ( $get_setup as $setup ) {
                ...
                $words .= '<li>'
                                . '<a href="#" class="delete-stream-setup" data-id="' . $setup->setup_id . '" data-template="new_tweets" data-stream-id="' . $stream[0]->stream_id . '">'
                                    . '<i class="fab fa-slack-hash"></i>'
                                    . $setup->setup_option
        ...
        $setup_data .= '<div class="row">'
                            ...
                            . '</div>'
                        . '</div>'
                        . '<input type="hidden" name="stream-template" value="new_tweets">'
    ...
    public function template_info() {
        ...
        return array (
            'displayed_name' => $this->CI->lang->line('new_tweets'),
            'icon' => '<i class="fab fa-twitter"></i>',
            'description' => $this->CI->lang->line('display_most_new_tweets'),
            'parent' => 'twitter'
        );
