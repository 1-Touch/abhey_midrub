 ...
 * Account Insights
 *
 * This file contains the Stream's Account Insights template 
 *
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.6
...
/*
 * Class Account_insights contains the Stream's Account Insights template 
 * 
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.6
 */
class Account_insights implements MidrubBaseUserAppsCollectionStreamInterfaces\Stream {
    ...
    public function template_content( $stream, $return = FALSE ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream->stream_id, 'account_insights');
        ...
        if ( $get_setup ) {
            ...
            $info = $graphNode->asArray();
            
            $content = json_encode(array(
                array(
                    'name' => $this->CI->lang->line('followers_count'),
                    'value' => $info['business_discovery']['followers_count'],
                    'background_color' => 'rgba(30, 99, 132, 0.6)',
                    'border_color' => 'rgba(30, 99, 132, 1)'
                ),
                array(
                    'name' => $this->CI->lang->line('media_count'),
                    'value' => $info['business_discovery']['media_count'],
                    'background_color' => 'rgba(60, 99, 132, 0.6)',
                    'border_color' => 'rgba(60, 99, 132, 1)'
                )            
            ));

            $insights = '<li class="row"' . $border_bottom_color . '>'
                            . '<div class="col-xl-12">'
                                . '<div>'
                                    . '<canvas id="stream-template-graph-' . $stream->stream_id . '" class="stream-template-graph" data-content="' . base64_encode($content) . '" style="width:100%" height="200"></canvas>'
                                . '</div>'
                            . '</div>'
                        . '</li>';
    ...
    public function stream_get_setup( $stream ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream[0]->stream_id, 'account_insights');
        ...
        $setup_data .= '<input type="hidden" name="stream-template" value="account_insights">'
    ...
    public function template_info() {
        ...
        return array (
            'displayed_name' => $this->CI->lang->line('account_insights'),
            'icon' => '<i class="icon-social-instagram"></i>',
            'description' => $this->CI->lang->line('account_insights_description'),
...
/* End of file Account_insights.php */
