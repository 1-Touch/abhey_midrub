 ...
 * VK Profile Template
 *
 * This file contains the Stream's VK Profile template 
 *
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.6
...
/*
 * VK_profile contains the Stream's VK_profile template 
 * 
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.6
 */
class VK_profile implements MidrubBaseUserAppsCollectionStreamInterfaces\Stream {
    ...
    public function template_content( $stream, $return = FALSE ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream->stream_id, 'vk_profile');
        ...
        if ( $posts ) {
            ...
            foreach ( $posts['response']['items'] as $post ) {
                ...
                $params = array(
                    'posts' => $network_details[0]->net_id . '_' . $post['id'],
                    'access_token' => $network_details[0]->token,
                    'v' => $this->version
                );
    ...
    public function template_info() {
        ...
        return array (
            'displayed_name' => $this->CI->lang->line('vk_profile'),
            'icon' => '<i class="fab fa-vk"></i>',
            'description' => $this->CI->lang->line('vk_profile_description'),
            'parent' => 'vk'
        );
