<?php
/**
 * Popular Tweets Template
 *
 * This file contains the Stream's Popular Tweets template 
 *
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.5
...
/*
 * Popular_tweets contains the Stream's Popular_tweets template 
 * 
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.5
 */
class Popular_tweets implements MidrubBaseUserAppsCollectionStreamInterfaces\Stream {
    ...
    public function template_content( $stream, $return = FALSE ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream->stream_id, 'popular_tweets');
        ...
        if ($this->CI->input->post()) {
            ...
            if ( $this->CI->form_validation->run() !== false ) {
                ...
                if ( $this->CI->stream_tabs_model->verify_stream_owner( $this->CI->user_id, $stream_id ) ) {
                    ...
                    $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream_id, 'popular_tweets');
    ...
    public function stream_get_setup( $stream ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream[0]->stream_id, 'popular_tweets');
        ...
        if ( $get_setup ) {
            ...
            foreach ( $get_setup as $setup ) {
                ...
                $words .= '<li>'
                                . '<a href="#" class="delete-stream-setup" data-id="' . $setup->setup_id . '" data-template="popular_tweets" data-stream-id="' . $stream[0]->stream_id . '">'
                                    . '<i class="fab fa-slack-hash"></i>'
                                    . $setup->setup_option
        ...
        $setup_data .= '<div class="row">'
                            ...
                            . '</div>'
                        . '</div>'
                        . '<input type="hidden" name="stream-template" value="popular_tweets">'
    ...
    public function template_info() {
        ...
        return array (
            'displayed_name' => $this->CI->lang->line('popular_tweets'),
            'icon' => '<i class="fab fa-twitter"></i>',
            'description' => $this->CI->lang->line('display_most_popular_tweets'),
            'parent' => 'twitter'
        );
...
/* End of file Popular_tweets.php */
