...
/*
 * Search_users contains the Stream's Search_users template 
 * 
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.7.5
 */
class Search_users implements MidrubBaseUserAppsCollectionStreamInterfaces\Stream {
    ...
    public function template_content( $stream, $return = FALSE ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream->stream_id, 'search_users');
        ...
        if ( $get_setup ) {
            ...
            foreach ( $get_setup as $set ) {
                ...
                $words[] = str_replace(' ', '%20', $set->setup_option);
                
            }
            
            $words = urlencode(implode('%20', $words));
            
        }
        
        $users = $connection->get('users/search',
            array(
                'q' => $words,
                'count' => 10
            )
        ...
        if ( $users ) {
            ...
            foreach ( $users as $user ) {
                ...
                $button = '<button type="button" class="btn btn-dark btn-subscribe mt-1 mb-2 stream-item-action" data-stream="' . $stream->stream_id . '" data-type="follow" data-id="' . $user->id . '">'
                                . '<i class="icon-user-following"></i> ' . $this->CI->lang->line('follow')
                            . '</button>';
                
                if ( $user->following ) {
                    
                    $button = '<button type="button" class="btn btn-dark btn-subscribe mt-1 mb-2 stream-item-action" data-stream="' . $stream->stream_id . '" data-type="unfollow" data-id="' . $user->id . '">'
                ...
                $all_users .= '<li class="row"' . $border_bottom_color . '>
                                <div class="col-xl-12">
                                    <img src="' . $user->profile_image_url_https . '" alt="User Avatar" class="img-circle">
                                    <div class="stream-post">
                                        <strong>
                                            <a href="https://twitter.com/' . $user->screen_name . '" target="_blank"' . $links_color . '>
                                                ' . $user->screen_name . '
                                            </a>
                                        </strong>
                                        <p>
                                        ...
                                        <div class="stream-post-footer">  
                                            <span' . $icons_color . '>
                                                <i class="fas fa-users"></i> ' . $user->followers_count . '
                                            </span>
                                            <span' . $icons_color . '>
                                                <i class="fas fa-user-friends"></i> ' . $user->friends_count . '    
                                            </span>
                                        </div>
                                    </div>
                                </div>
        ...
        if ($this->CI->input->post()) {
            ...
            if ( $this->CI->form_validation->run() !== false ) {
                ...
                if ( $this->CI->stream_tabs_model->verify_stream_owner( $this->CI->user_id, $stream_id ) ) {
                    ...
                    $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream_id, 'search_users');
    ...
    public function stream_get_setup( $stream ) {
        ...
        $get_setup = (new MidrubBaseUserAppsCollectionStreamHelpers\Streams_setup)->streams_setup_get($stream[0]->stream_id, 'search_users');
        ...
        if ( $get_setup ) {
            ...
            foreach ( $get_setup as $setup ) {
                ...
                $words .= '<li>'
                                . '<a href="#" class="delete-stream-setup" data-id="' . $setup->setup_id . '" data-template="search_users" data-stream-id="' . $stream[0]->stream_id . '">'
                                    . '<i class="fab fa-slack-hash"></i>'
                                    . $setup->setup_option
        ...
        $setup_data .= '<div class="row">'
                            ...
                            . '</div>'
                        . '</div>'
                        . '<input type="hidden" name="stream-template" value="search_users">'
    ...
    public function template_info() {
        ...
        return array (
            'displayed_name' => $this->CI->lang->line('search_users'),
            'icon' => '<i class="fab fa-twitter"></i>',
            'description' => $this->CI->lang->line('search_users_description'),
            'parent' => 'twitter'
        );
