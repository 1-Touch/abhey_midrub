<?php
/**
 * Stream Networks Model
 *
 * PHP Version 5.6
 *
 * Stream Networks file contains the Networks Model
 *
 * @category Social
 * @package  Midrub
...
/**
 * Stream_networks_model class - operates the networks table.
 *
 * @category Social
 * @package  Midrub
 * @author   Scrisoft <asksyn@gmail.com>
 ...
 * @link     https://www.midrub.com/
 */
class Stream_networks_model extends CI_MODEL {
    ...
    public function delete_account_records( $user_id, $account_id ) {
        ...
        if ( $query->num_rows() > 0 ) {
            ...
            foreach ( $response as $res ) {
                $this->db->delete('stream_tabs_streams', array('stream_id' => $res->stream_id));
                $this->db->delete('stream_tabs_streams_setup', array('stream_id' => $res->stream_id));
                $this->db->delete('stream_history', array('stream_id' => $res->stream_id));
